...
function GatewayService(options) {
    ...
    function performTransaction(request, response, callback) {
        ...
        if (request.hasOwnProperty('gatewayURL') 
            && request.gatewayURL !== null) {
            fullURL = [request.gatewayURL];
        }
        
        if(fullURL == null && request.hasOwnProperty('embeddedFieldsToken') 
           && request.embeddedFieldsToken !== null) {
           fullURL = [request.embeddedFieldsToken];
        }
